...
| R-12 | functional | must | Every operation is scoped to the caller's own resources; an admin role may act on any resource (assumed by the engine). | — |
...
### I-16 — Reading domain interface

- **kind**: module · **owner**: C-16 · **stability**: draft
- Provided by Reading domain. Operations are the state transitions and creations the requirements name; add queries as the surfaces need them.
...
### E-5 — Reading (owner C-16)

Domain entity read from R-1, R-2. Invariant (R-7): durability: no reading lost.
