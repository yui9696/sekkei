...
|---|---|---|---|---|---|
| 1 | load | Q-rate | 20 requests/s sustained, 10x at peak. | evidence: count 2,000 — Derived from the largest stated count (2,000) at 1 request per 100 items per second. | State the measured or expected rate; capacity estimates and the queue decision change. |
...
|---|---|---|---|
| R-3 | C-13 Public HTTP API, C-6 Domain core | surface+core | human actor 'employees': a use case |

## 2. Capacity estimates

| estimate | value | formula | inputs |
|---|---|---|---|
| requests per day | 1.73 M | rate × 86,400 s | 20 (R-16) |
| storage growth per day (requests) | 3.54 GB | rate × 86,400 × record size | 20 (R-16); assumed 2048 bytes per record |
| storage after 30 days (requests) | 106.17 GB | daily growth × 30 | same inputs |
| backlog after a 1 h downstream outage | 72 k requests | rate × outage seconds | 20 (R-16); outage length assumed |
| concurrent handlers to sustain the rate (requests) | 4 | Little's law: rate × mean service time | 20 (R-16); mean service time assumed 200 ms |
| in-flight items at the latency target | 200 | rate × latency target (Little's law upper bound) | 20 (R-16) × 10 min (R-8) |
| number of users | 2 k | stated | 2000 users (R-9) |
| average rate per user (if evenly spread) | 0.01/s | rate ÷ count | 20 ÷ 2000 |
...
- Total effort: **23 person-days**; critical path **13 days**; with a team of 4: **about 13 working days** (3 weeks).
...
| C-13 Public HTTP API | spoofing | Requests without a verified caller identity reach domain operations. | Authenticate every route in one middleware; deny by default. | every route returns 401 without credentials |
| C-13 Public HTTP API | tampering | Malformed or oversized bodies reach the core. | Schema-validate and size-limit at the surface; reject before parsing fully. | fuzz the body; oversize returns 413 |
| C-13 Public HTTP API | denial_of_service | A single caller saturates the service. | Per-caller rate limit and request timeouts. | burst from one key returns 429; others unaffected |
| C-13 Public HTTP API | information_disclosure | Stack traces or internal ids leak in error responses. | Map exceptions to fixed error shapes; log details server-side only. | no traceback text in any 4xx/5xx body |
...
- Japanese words the glossary does not know were dropped: 「め」, 「タイトル」, 「トランスコード」, 「互換」, 「使う」, 「受け」, 「委ねる」, 「提供」, 「日」, 「英語」, 「語」, 「説明」, 「部」, 「配置」.
...
- D-9 Assumed answer: load (Q-rate): **20 requests/s**
...
- Patterns recognised: crud_api, notification, observability, auth, search, file_storage, audit_log, import_export, media, i18n
...
| R-2 | functional | could | search, media | durability | — |
| R-3 | functional | must | **none** | — | — |
| R-4 | functional | must | import_export | — | — |
| R-5 | functional | must | notification | — | — |
| R-6 | functional | must | audit_log | compliance | — |
| R-7 | functional | must | notification, i18n | — | — |
| R-8 | nonfunctional | must | file_storage, media | performance | latency at 2 GB <= 10 min min |
| R-9 | nonfunctional | must | media | consistency, scalability | number of users 2000 users users |
| R-10 | nonfunctional | must | — | performance | p95 latency <= 300 ms ms |
| R-11 | nonfunctional | must | — | consistency, durability | records lost across a process crash = 0 records |
...
| R-16 | nonfunctional | should | — | performance | sustained rate 20 requests /s requests /s |
...
| 画面とメールは社員のロケール(日本語・英語)で表示する。 | The system must view pages email employees locale items. |
...
| 動画のトランスコード(既存サービスに委ねる)。 | Videos existing service. |
| 外部受講者(社外)への提供。 | Outside learners external. |

Words the glossary does not know (dropped from the English; add them to the text in English or extend the glossary): 「め」, 「タイトル」, 「トランスコード」, 「互換」, 「使う」, 「受け」, 「委ねる」, 「提供」, 「日」, 「英語」, 「語」, 「説明」, 「部」, 「配置」
