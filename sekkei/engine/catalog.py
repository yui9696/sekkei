...
DECISIONS: dict[str, DecisionPoint] = {d.key: d for d in [
    ...
    DecisionPoint("redundancy", "Redundancy for the availability target",
                  ...
                  [Option("Two or more interchangeable instances per role behind the ingress, health checks, rolling deploys",
                   ...
                   Option("Active-active across two regions", ["survives a regional outage"], ["data replication and conflict handling", "cost"],
                          {"availability": 3, "simplicity": 0, "cost": 0, "scalability": 3}, excludes=["single_region"])],
    ...
    DecisionPoint("vector_store", "Vector index for semantic search",
                  ...
                  ["search", "store"], trigger=["vector_db", "ml_inference"]),
...
PATTERNS: list[Pattern] = [
    ...
    Pattern("payments", "Payments", [(r"\bpayments?\b", 1), (r"\bpayment (?:provider|gateway|method)s?\b|\bprocess(?:es|ing)? payments?\b|\btake payments?\b", 3), (r"\binvoice", 2), (r"\bbilling\b", 2), (r"\bstripe\b|\bcharged?\b|\bcheckout\b", 2), (r"\bpay(?:s|ing)? (?:for|by|with)\b", 2)],
    ...
    Pattern("ml_inference", "Model inference", [(r"\binference\b", 3), (r"\bpredict", 2), (r"\bmodel\b", 1), (r"\bembedding", 2), (r"\bclassif", 1), (r"\bllm\b", 2), (r"\bsummar", 1)],
            ["model", "core"], [], [], ["vector_store"], [], "Predictions are served from a versioned model."),
    ...
    Pattern("compliance_data", "Personal data protection", [(r"\bgdpr\b|\bhipaa\b|\bpci\b|\bappi\b", 3), (r"\bpersonal data\b|\bpii\b", 3), (r"\bconsent\b", 2), (r"\bdelete (?:their|my|all|a person's) data\b|\bright to (?:erasure|be forgotten)\b", 3), (r"\banonymi[sz]", 2), (r"\bretention\b", 1), (r"\bdata subject\b", 3)],
    ...
    Pattern("legacy_integration", "Integration with an existing system", [(r"\blegacy\b", 3), (r"\bexisting (?:system|erp|crm|database|application)\b", 3), (r"\berp\b|\bcrm\b|\bmainframe\b", 2), (r"\bmigrat(?:e|ed|ion) (?:from|data)\b", 2), (r"\bsystem of record\b", 2), (r"\bintegrat(?:e|es|ed|ion) with\b", 1)],
