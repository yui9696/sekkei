...
PATTERNS: list[Pattern] = [
    ...
    Pattern("legacy_integration", "Integration with an existing system", [(r"\blegacy\b", 3), (r"\bexisting (?:\w+ ){0,3}(?:system|erp|crm|database|application)\b", 3), (r"\berp\b|\bcrm\b|\bmainframe\b", 2), (r"\bmigrat(?:e|ed|ion) (?:from|data)\b", 2), (r"\bsystem of record\b", 2), (r"\bintegrat(?:e|es|ed|ion) with\b", 1)],
...
CONSTRAINT_TOKENS: list[tuple[str, str]] = [
    ...
    (r"command[- ]line\b|\bcli\b|\bthe tool runs\b", "cli_tool"), (r"\bon[- ]prem", "on_prem"), (r"\bnightly\b|\bevery night\b|\bnightly batch\b|\bonce a day\b", "nightly_batch"),
    ...
    (r"\bdata residency\b|\bresidency\b|\bmust (?:stay|remain|be stored) in (?:the )?(?:eu|europe|japan|us|country)\b|\bデータ所在\b", "data_residency"),
    (r"\bheroku\b|\brender\.com\b|\bfly\.io\b|\bcloud run\b|\bapp service\b|\bpaas\b", "paas"), (r"\bslack\b|\bteams\b|\bdiscord\b", "chat_channel"),
    (r"\bexactly[- ]once at the row level\b|\bidempotent(?:ly)?\b|\bre-?run\b", "idempotent_loads"), (r"\bno database\b|\bwithout a database\b", "no_database"), (r"\bno network\b|\boffline\b|\bair[- ]gapped\b", "no_network"), (r"\bwindows\b", "windows"), (r"\blinux\b", "linux"), (r"\bmacos\b", "macos"),
]
