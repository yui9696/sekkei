...
PATTERNS: list[Pattern] = [
    ...
    Pattern("signing", "Request signing", [(r"\bsign(?:ed|ing|ature)?\b(?! urls?\b)(?!-in)(?! in\b)(?! up\b)(?! (?:the |a |an )?(?:consent|form|document|contract|agreement|pdf|off))", 1), (r"\bsigned (?:json |http )?(?:events?|requests?|payloads?|bodies|body|webhooks?)\b|\brequest signing\b|\bsigning secret\b", 3), (r"\bhmac\b", 2), (r"\bsecret", 1), (r"\brotat", 1)],
    ...
    Pattern("cli_tool", "Command-line tool", [(r"command[- ]line", 3), (r"\bcli tool\b|\bthe cli\b", 3), (r"\bcli\b", 1), (r"\bterminal\b(?! units?)(?! equipment)", 1), (r"\bin the terminal\b|\bterminal (?:window|session|emulator)\b", 2), (r"\bcommand[- ]line flags?\b|\s--[a-z]|\bsubcommands?\b", 2), (r"\bstdin\b|\bstdout\b|\bstderr\b|\bexit(?:s)? (?:with )?code", 2), (r"\bthe tool\b|\bthe user runs\b", 1)],
    ...
    Pattern("payments", "Payments", [(r"\bpayments?\b", 1), (r"\bpayment (?:provider|gateway|method)s?\b|\bprocess(?:es|ing)? payments?\b|\btake payments?\b", 3), (r"\binvoice", 2), (r"\bbilling\b", 1), (r"\bstripe\b|\bcharged?\b(?! the (?:battery|device))|\bcheckout\b", 2), (r"\bpay(?:s|ing)? (?:for|by|with)\b", 2)],
