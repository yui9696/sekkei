...
ARCHETYPES: dict[str, Archetype] = {a.key: a for a in [
    ...
    Archetype("payout", "Payouts", "Initiates transfers to recipients (bank account, wallet) through the payment provider's payout API, idempotent per claim/settlement; records every instruction and its outcome.",
              layer=1, needs=["store"], iface_kind="module", ops=[
                  OpT("pay_out", [("recipient", "Recipient"), ("amount", "Money"), ("reference", "str")], "Payout(id, status)", ["DuplicatePayoutError", "ProviderError"], pre="no payout with the same reference exists", post="instruction recorded before the provider is called"),
                  OpT("status", [("payout_id", "str")], "PayoutStatus"),
              ]),
    ...
    Archetype("data_protection", "Data protection", "Retention schedules, deletion and export requests for a person's data, consent records; runs the deletions and proves them.",
              kind="job", layer=2, needs=["store", "audit"], iface_kind="module", ops=[
                  ...
                  OpT("run_due", [("now", "datetime")], "int completed", description="deletes or anonymises the subject's personal data in every store except append-only audit/chain-of-custody records, which are pseudonymised; audit entry per subject"),
...
DECISIONS: dict[str, DecisionPoint] = {d.key: d for d in [
    DecisionPoint("queue_tech", "Work queue technology",
                  ...
                  [Option("PostgreSQL table with SELECT ... FOR UPDATE SKIP LOCKED",
                          ...
                          {"durability": 3, "simplicity": 3, "performance": 2, "isolation": 2, "scalability": 2, "cost": 3, "operability": 3}, bonus_when=["postgres"], excludes=["no_database", "cli_tool", "existing_broker"], max_rate=10000),
    ...
    DecisionPoint("store_tech", "Primary store",
                  ...
                  [Option("PostgreSQL", ["transactions", "indexes and JSON", "widely available"], ["operational dependency"],
                   ...
                   Option("Redis for the hot state (as stated) with a relational store for durable records", ["the stated home of the hot data", "sub-millisecond reads"], ["two stores to keep consistent", "Redis durability depends on AOF/fsync"],
                          {"durability": 2, "performance": 3, "simplicity": 2, "scalability": 3, "cost": 2, "compliance": 2, "operability": 2}, needs=["redis_primary"], bonus_when=["redis_primary"]),
                   ...
                   Option("In-memory", ["fastest", "trivial"], ["lost on restart"],
                          {"durability": 0, "performance": 3, "simplicity": 3, "scalability": 0, "cost": 3, "compliance": 0, "operability": 2}, excludes=["containers", "multi_instance", "mysql", "postgres", "durable_required", "on_prem"])],
                  ["store"], trigger=["*"]),
    ...
    DecisionPoint("auth_scheme", "Caller authentication",
                  ...
                  [Option("API keys per customer, hashed at rest, sent as a bearer token", ["simple", "scriptable"], ["no delegation or expiry unless added"],
                   ...
                   Option("Session tokens issued by the platform's own account service to game/mobile clients (device-bound, short-lived, refreshable)", ["fits clients without a browser", "revocable per device"], ["a token service to run"],
                          {"security": 3, "simplicity": 1, "usability": 3}, needs=["game_client"], bonus_when=["game_client"]),
                   Option("No account: a reference number plus a knowledge factor (date of birth) resumes a saved application; every attempt rate-limited and logged", ["no credentials to manage for occasional users", "meets 'no account or email required'"], ["a reference number can be shared; scope what it unlocks"],
                          {"security": 1, "simplicity": 3, "usability": 3}, needs=["no_account_auth"], bonus_when=["no_account_auth"]),
    ...
    DecisionPoint("data_residency", "Where personal data may live",
                  ...
                  [Option("Single region in the stated jurisdiction for every store, queue, backup and log; no cross-region replication of personal data", ["simple to audit", "no transfer mechanism needed"], ["a regional outage is a full outage"],
                          {"compliance": 3, "simplicity": 3, "availability": 1, "cost": 3}, bonus_when=["data_residency"], excludes=["multi_jurisdiction"]),
                   ...
                   Option("One deployment (stores, queues, backups, logs) per jurisdiction; each tenant pinned to one, with no cross-jurisdiction replication of personal data", ["each customer's data stays where it must", "auditable per region"],
                          ["N deployments to operate", "cross-region features (global search, reporting) need explicit design"],
                          {"compliance": 3, "simplicity": 1, "availability": 2, "cost": 1}, needs=["multi_jurisdiction"], bonus_when=["multi_jurisdiction"])],
    ...
    DecisionPoint("api_style", "API style", "Clients need a programmable surface.",
                  [Option("REST/JSON over HTTP", ["universal tooling", "cacheable reads"], ["over-fetching on nested data"],
                          {"usability": 3, "simplicity": 3, "performance": 2}, bonus_when=["rest_api"]),
                   Option("gRPC", ["typed contracts", "streaming"], ["browser and debugging friction"],
                          {"usability": 2, "simplicity": 1, "performance": 3}, bonus_when=["grpc_api"]),
                   Option("GraphQL", ["flexible queries"], ["complexity budget for a small team"],
                          {"usability": 2, "simplicity": 1, "performance": 2}, bonus_when=["graphql_api"])],
    ...
    DecisionPoint("tenancy_model", "Tenant isolation model",
                  ...
                  [Option("Shared schema with a tenant_id column on every table, enforced by the tenant context (and row-level security where available)",
                   ...
                   Option("Database (or deployment) per tenant", ["complete isolation", "per-tenant residency"],
                          ...
                          {"isolation": 3, "simplicity": 1, "cost": 0, "compliance": 3, "scalability": 1, "operability": 1, "security": 3}, bonus_when=["single_tenant"])],
    ...
    DecisionPoint("legacy_integration", "Integration with the existing system",
                  ...
                  [Option("API façade (anti-corruption layer) calling the legacy system's interfaces, with a translation layer and retries",
                          ...
                          {"simplicity": 2, "durability": 2, "operability": 3, "consistency": 2, "cost": 3}, bonus_when=["legacy_api"]),
                   ...
                   Option("Change data capture from the legacy database", ["near real time", "no legacy code changes"],
                          ...
                          {"simplicity": 1, "durability": 3, "operability": 1, "consistency": 3, "cost": 1}, excludes=["legacy_api"]),
                   Option("Events over the existing message topics (the stated integration points); no new synchronous calls", ["decoupled", "already operated"],
                          ["at-least-once: consumers must be idempotent", "schema of the topics to govern"],
                          {"simplicity": 2, "durability": 3, "operability": 3, "consistency": 2, "cost": 3}, needs=["broker"], bonus_when=["broker_integration"])],
...
PATTERNS: list[Pattern] = [
    ...
    Pattern("signing", "Request signing", [(r"\bsign(?:ed|ing|ature)?\b(?! urls?\b)(?!-in)(?! in\b)(?! up\b)(?! (?:the |a |an )?(?:consent|form|document|contract|agreement|pdf|off))", 1), (r"\bsigned (?:json |http )?(?:events?|requests?|payloads?|bodies|body|webhooks?)\b|\brequest signing\b|\bsigning secret\b", 3), (r"\bsigned by (?:the )?[a-z]+\b|\bpgp[- ]signed\b|\bcosign\b|\bsigned manifest\b", -2), (r"\bhmac\b", 2), (r"\bsecret", 1), (r"\brotat", 1)],
    ...
    Pattern("chat_notification", "Chat-channel notifications", [(r"\bslack\b(?! exports?)|\bmicrosoft teams\b|\bteams (?:channel|message|bot|webhook|notification)s?\b|\bdiscord\b|\bchat channel\b|\bpost(?:s|ed)? to (?:slack|teams|a channel)\b", 3), (r"#[a-z][a-z0-9_-]+", 1)],
    ...
    Pattern("cli_tool", "Command-line tool", [(r"command[- ]line", 3), (r"\bcli tool\b|\bthe cli\b", 3), (r"\bcli\b", 1), (r"\bterminal\b(?! units?)(?! equipment)", 1), (r"\bin the terminal\b|\bterminal (?:window|session|emulator)\b", 2), (r"\bcommand[- ]line flags?\b|(?<![-a-z])\s--[a-z][a-z-]*\b(?![^\n]{0,24}-->)|\bsubcommands?\b", 2), (r"\bstdin\b|\bstdout\b|\bstderr\b|\bexit(?:s)? (?:with )?code", 2), (r"\bthe tool\b|\bthe user runs\b", 1)],
            ["cli", "core", "config"], [], ["cli_run"], ["store_tech"], [], "A command-line front end over the core."),
    Pattern("local_ui", "Local user interface", [(r"touch ?panel", 3), (r"\bon[- ]screen\b|\bthe screen\b|\bscreens? (?:show|display)", 2), (r"\bscreen\b(?![- ]fail)", 1), (r"\bgui\b", 3), (r"\bdisplay(?:s|ed)?\b", 1), (r"\bchart\b|\bgraph\b", 1), (r"\bbutton", 2), (r"\bpanel\b", 1)],
    ...
    Pattern("payments", "Payments", [(r"\bpayments?\b", 1), (r"\bpayment (?:provider|gateway|method)s?\b|\bprocess(?:es|ing)? payments?\b|\btake payments?\b", 3), (r"\binvoice", 2), (r"\bbilling\b", 1), (r"\bstripe\b|\bcharged?\b(?! the (?:battery|device))|\bcheckout\b", 2), (r"\bpay(?:s|ing)? (?:by card|with (?:a )?card|by (?:credit|debit) card|online)\b|\bcustomers? pays?\b", 2)],
            ["payments", "psp", "store"], [], [], [], ["duplicate_delivery"], "Charges and invoices through a payment provider."),
    Pattern("payouts", "Payouts to recipients", [(r"\bpayouts?\b|\bpay(?:s|ing)? out\b|\bdisburs", 3), (r"\bsettle(?:s|d|ment)? to (?:the )?(?:seller|recipient|vet|clinic|policyholder)", 3), (r"\bbank (?:account|transfer)\b|\biban\b", 1), (r"\breimburs", 2)],
            ["payout", "store"], [], [], [], ["duplicate_delivery"], "Money is sent to recipients' accounts; never twice."),
    ...
    Pattern("audit_log", "Audit log", [(r"\baudit(?:ed|ing|s)?\b(?![- ]?(?:tool|repo|scan|script))|\baudit (?:trail|log|record)s?\b", 3), (r"who did (?:it|what)", 3), (r"\bwho (?:took|made|approved|changed|performed)\b", 2), (r"\bfor (?:regulators|auditors|compliance)\b", 2), (r"\bhistory\b", 1), (r"\brecorded with\b", 1), (r"\bmodel version\b", 1), (r"\bchange log\b|\bchangelog\b", 2)],
...
CONSTRAINT_TOKENS: list[tuple[str, str]] = [
    ...
    (r"\bexisting (?:rabbitmq|kafka|sqs|nats|pulsar|message broker|broker|queue)\b|\b(?:rabbitmq|kafka|sqs|nats) (?:cluster |already )?(?:exists?|is available|available|already)\b|\bwe (?:run|have|use) (?:rabbitmq|kafka|sqs)\b", "existing_broker"),
    (r"\brest(?:ful)?\b|\bhttps? api\b|\bjson api\b|\bopenapi\b|\bswagger\b", "rest_api"), (r"\bgrpc\b|\bprotobuf\b", "grpc_api"), (r"\bgraphql\b", "graphql_api"),
    (r"\bsoap api\b|\bexposes (?:a |an )?(?:soap|rest|graphql|grpc)? ?api\b|\bvia its (?:rest |soap )?api\b|\b(?:soap|rest) (?:endpoint|interface)s?\b", "legacy_api"),
    (r"\b(?:kafka|rabbitmq|sqs|pub/?sub) topics?\b.{0,40}\bintegration points?\b|\bintegration points? (?:are|is) (?:the )?(?:kafka|rabbitmq|existing)\b|\bdo not add synchronous calls\b", "broker_integration"),
    (r"\bruns? per customer\b|\bsingle[- ]tenant\b|\bone deployment per (?:customer|client|tenant)\b|\bper-customer deployment\b|\bdeployed per customer\b", "single_tenant"),
    (r"\b(?:live|lives|stored|kept|held) in redis\b|\bredis (?:is|as) the (?:primary |main )?store\b", "redis_primary"),
    (r"\bgame clients?\b|\bplayers?\b|\bconsoles?\b|\bgame servers?\b", "game_client"),
    ...
    (r"\bno account (?:or email address )?(?:is )?(?:required|needed)\b|\breference number and (?:their )?date of birth\b|\bwithout (?:creating )?an account\b|\bdo not have accounts\b|\bparticipants do not have accounts\b", "no_account_auth"),
    (r"\bdata residency\b|\bresidency\b|\bmust (?:stay|remain|be stored|be kept) in (?:the )?[a-z]+(?: region)?\b|\bmust not leave (?:the )?[a-z]+(?: region| project)?\b|\bdata (?:stays|remains|stay|remain) in\b|\bto stay in [a-z]+\b|\bdomestic region\b|\brestricted to (?:the )?\w+ region\b|\b(?:eu|uk|japan|tokyo|frankfurt|sweden|finland|nordic) regions? only\b|\bデータ所在\b|\btokyo region\b", "data_residency"),
    (r"\b(?:eu|european) customers?'? data\b.{0,40}\b(?:us|american|us customers?)\b|\bper[- ]jurisdiction\b|\bregion per (?:customer|tenant|jurisdiction)\b|\bpinned to (?:a|the|its) region\b|\bpin .{0,30} tenant to\b", "multi_jurisdiction"),
...
LANGUAGE_TOKENS: list[tuple[str, str]] = [
    (r"\bpython\b", "python"), (r"\btypescript\b|\bnode(?:\.js)?\b|\bjavascript\b|\bsuitescript\b|\bnestjs\b|\bdeno\b|\bbun\b", "typescript"), (r"\bgolang\b|\bgo 1\.\d+\b|\bin go\b|\bwritten in go\b|\bgo (?:service|binary|module)s?\b", "go"),
