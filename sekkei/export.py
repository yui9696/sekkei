...
def openapi(d: Design) -> dict:
    ...
    for iface in d.interfaces:
        ...
        for op in iface.operations:
            ...
            key = f"{method} {path}"
            if key in used:
                # the same route derived on two surfaces (Admin API and Ingest API from one sentence): one path item, noted
                notes.append(f"{key} is declared by more than one interface ({iface.name}); one path item is emitted — merge the interfaces or rename the route")
                continue
            used.add(key)
            params = []
            body_props: dict = {}
